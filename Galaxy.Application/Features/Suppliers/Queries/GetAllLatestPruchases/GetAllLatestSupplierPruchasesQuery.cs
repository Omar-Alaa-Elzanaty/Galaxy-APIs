...
namespace Galaxy.Application.Features.Suppliers.Queries.GetAllLatestPruchases
{
    ...
    internal class GetAllLatestPruchasesQueryHandler : IRequestHandler<GetAllLatestSupplierPruchasesQuery, PaginatedResponse<GetAllLatestPruchasesQueryDto>>
    {
        ...
        public async Task<PaginatedResponse<GetAllLatestPruchasesQueryDto>> Handle(GetAllLatestSupplierPruchasesQuery query, CancellationToken cancellationToken)
        {
            ...
            var suppliersLastPruchases = _unitOfWork.Repository<Supplier>().Entities().OrderByDescending(x => x.CreationDate).Select(x => new
            {
                SupplierId = x.Id,
                x.Name,
                Details = x.Invoices.OrderByDescending(i => i.CreationDate).FirstOrDefault()
            });

            if (!query.KeyWord.IsNullOrEmpty())
            {
                suppliersLastPruchases = suppliersLastPruchases.Where(x => x.Name.ToLower().Contains(query.KeyWord!.ToLower()));
            }

            if (query.LatestPurchasesColumn != null)
            {
            ...
            suppliersLastPruchases = query.PageNumber > 1 ? suppliersLastPruchases.Skip((query.PageNumber - 1) * query.PageSize).Take(query.PageSize)
                                    : suppliersLastPruchases.Take(query.PageSize);
